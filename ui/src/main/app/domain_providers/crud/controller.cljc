(ns app.domain-providers.crud.controller
  (:require [re-frame.core :refer [reg-event-fx]]
            ...
            [app.domain-providers.crud.form :as form]
...
(reg-event-fx
 ...
 (fn [_ [pid phase {:keys [id]}]]
   (case phase
     :init   {:fx [[:xhr/fetch {:uri     (str "/api/domain_providers/" id)
...
(reg-event-fx
 ...
 (fn [{db :db} [_ payload]]
   (let [id (-> db :fragment-params :id)
     ...
     {:db (assoc-in db [pid/common :save-message] (if id "Провайдер домена успешно сохранен" "Провайдер домена успешно создан"))
      :xhr/fetch {:uri     (cond-> "/api/domain_providers"
                             id (str "/" id))
